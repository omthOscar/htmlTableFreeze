...
$.fn.Freeze = function(headRows, footRows, leftColumn, rightColumn) {
	...
	if (headRows > 0) {
		...
		$(newTable).css("width",width);
		$("#headTableDiv").css("height", $(newTable).outerHeight(true) + 1);
		$("#headTableDiv").css("width", width);
	...
	if (leftColumn > 0) {
		...
		var width = $(newTable).css("width");
		var height = $(newTable).css("height");
		$("#leftTableDiv").css("width", width);
		$("#leftTableDiv").css("height", height);
	}
...
var copyLeftColumn = function(toTable, sourceTable, startRows, endRows,
	...
	var columnWidths = [];
	var tr = $(sourceTable).find("tr").eq(0);
	for ( var i = 0; i < columnNums; i++) {
		var td = $(tr).find("td").eq(i);
		columnWidths[i] = $(td).attr("width");
	}
	...
	for ( var i = startRows; i < rowNums; i++) {
		...
		for ( var column = 0; column < columnNum; column++) {
			...
			var newTD = td.clone();
			$(newTD).attr("width",columnWidths[column])
			$(newTr).append(newTD);
